...
def get_clear_str(string): # функция, которая чистит строку, оставляя только буквы
    return " ".join(re.findall("\w+", string))


"""

Парсер сайта https://tgstat.ru/ с помощью библиотеки BeautifulSoup

На выходе получаем файл users.svg со статистикой по топовым телеграм каналам в 47 категориях.
...
ratings = [link.get("href") for link in links if link.get("href") and link.get("href").startswith("/ratings")][1:] # получаем ссылки на страницы с топами по 47 категориям
data = []
t_rating = 1
for rating in ratings: # заходим на страницу каждой категории, откуда будем брать каналы
    try:
        url = base_url + rating
        url_t = url.split("?")
        ...
        users_links = [a.get("href") for a in channels_list.findAll("a")] # ссылки на каналы в топе
        t_user = 1
        for user_link in users_links: # заходим на страницу канала, чтобы взять статистику
            try:
                ...
                Анализируя html элементы парсим с них определнную информацию
                ...
                Приватные каналы и публичные имеют разную статистику, парсим по-разному
                ...
                data.append(row)
                pd.DataFrame(data).to_csv("users.csv") #сохраняем результат
